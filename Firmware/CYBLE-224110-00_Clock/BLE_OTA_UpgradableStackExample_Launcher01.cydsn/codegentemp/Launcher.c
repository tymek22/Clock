...
*******************************************************************************/
    ...
    #pragma location=".bootloader"
           ...
           const uint8  CYCODE Launcher_Checksum = 0u;
#elif defined (__ICCARM__)
    __root const uint8  CYCODE Launcher_Checksum = 0u;
#endif  /* defined(__ARMCC_VERSION) || defined (__GNUC__) || defined (__C51__) */
const uint8  CYCODE *Launcher_ChecksumAccess  = (const uint8  CYCODE *)(&Launcher_Checksum);

#if defined(__ARMCC_VERSION) || defined (__GNUC__)
    __attribute__((section (".bootloader"), used))
    const uint32 CYCODE Launcher_SizeBytes = 0xFFFFFFFFu;
#elif defined (__ICCARM__)
    #pragma location=".bootloader"
    __root const uint32 CYCODE Launcher_SizeBytes = 0xFFFFFFFFu;
#elif defined (__C51__)
    const uint32 CYCODE Launcher_SizeBytes = 0xFFFFFFFFu;
#endif  /* defined(__ARMCC_VERSION) || defined (__GNUC__) */

const uint32 CYCODE *Launcher_SizeBytesAccess = (const uint32 CYCODE *)(&Launcher_SizeBytes);
 ...
 */
uint8 Launcher_isBootloading = Launcher_BOOTLOADING_COMPLETED;
#endif /*CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER*/
 ...
 */
volatile uint8 Launcher_activeApp = Launcher_MD_BTLDB_ACTIVE_NONE;
#else /*SINGLE-APP BOOTLOADER*/
    #define Launcher_activeApp      (Launcher_MD_BTLDB_ACTIVE_0)
 ...
 */
static const uint8 CYCODE Launcher_securityKey[Launcher_SECURITY_KEY_LENGTH] = {0x11u, 0x22u, 0x33u, 0x44u, 0x55u, 0x66u};
 ...
 */
static Launcher_callback_type Launcher_callback = NULL;
...
***************************************/
#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
    static void  Launcher_LaunchApplication(void) CYSMALL \
                    ; 
#if(!CY_PSOC3)
    /* Implementation for PSoC 3 resides in Bootloader_psoc3.a51 file.  */
    static void     Launcher_LaunchBootloadable(uint32 appAddr);
#endif  /* (!CY_PSOC3) */
#endif /* CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER */ 

#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)
    static cystatus Launcher_WritePacket(uint8 status, uint8 buffer[], uint16 size) CYSMALL \
                                    ;

    static uint16   Launcher_CalcPacketChecksum(const uint8 buffer[], uint16 size) CYSMALL \
                                    ;
                                    
    #if (0u != Launcher_CMD_VERIFY_FLS_ROW_AVAIL)
    static uint8 Launcher_VerifyRow(uint32 flashStart, const uint8* ramStart, uint16 size) \
                CYSMALL ;                                
    #endif /*(0u != Launcher_CMD_VERIFY_FLS_ROW_AVAIL)*/            

#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)                                    
    static void Launcher_HostLink(uint8 timeOut) CYLARGE ;
#endif /* (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) */                                    
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)*/
                                    
#if (((0u != Launcher_DUAL_APP_BOOTLOADER) && (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)) \
                                          || (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))
    static uint8 Launcher_CheckImage(uint8 appNumber, uint8 arrayNumber, uint16 row, uint16 rowNumInArray) CYSMALL \
                                    ;                                    
#endif /*(((0u != Launcher_DUAL_APP_BOOTLOADER) && (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)) \
                                          || (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))*/

#if ((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || (0u != Launcher_DUAL_APP_BOOTLOADER)) 
    static void Launcher_SetActiveAppInMetadata(uint8 activeApp) CYSMALL \
                ;
#endif /* (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || (0u != Launcher_DUAL_APP_BOOTLOADER) */                     

#if ((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || \
     (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_MULTIAPPBOOTLOADER) || \
     (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER))
    static uint8 Launcher_GetActiveAppFromMetadata(void) CYSMALL \
                ;                     
#endif /*((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || \
          (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_MULTIAPPBOOTLOADER) || \
          (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER)) */
...
#if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER)
#if ((0u != Launcher_COPIER_AVAIL) && (CY_PSOC4)) 
    static cystatus Launcher_CopyRow(uint16 srcRowNum, uint16 dstRowNum) \
                 CYSMALL ;
    static void Launcher_Copier(void) CYSMALL ;
#endif /*(0u != Launcher_COPIER_AVAIL) && (CY_PSOC4)*/
#endif /*(CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER)*/
...
*/
#if ((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || \
                                  (0u != Launcher_DUAL_APP_BOOTLOADER))                                          
/*******************************************************************************
* Function Name: Launcher_SetActiveAppInMetadata
****************************************************************************//**
* \internal
*
* \brief
...
{
    if (Launcher_MD_BTLDB_ACTIVE_NONE > activeApp)
    {
        uint8 CYDATA idx;    
        for(idx = 0u; idx < Launcher_MAX_NUM_OF_BTLDB; idx++)
        {
            (void)Launcher_SetFlashByte((uint32) Launcher_MD_BTLDB_ACTIVE_OFFSET(idx),
                                                (uint8)(idx == activeApp));
        }
    }    
}
     ...
     (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER))                   
/*******************************************************************************
* Function Name: Launcher_GetActiveAppFromMetadata
****************************************************************************//**
* \internal
*
* \brief
...
*    then Launcher_MD_BTLDB_ACTIVE_NONE (0x02) is returned.
*
* \return
*   The number of active applications. In the case of error:
*   -  Launcher_MD_BTLDB_ACTIVE_NONE (0x02) - neither application is initialized;
*   -  Launcher_BOTH_ACTIVE (0x03) - both applications are set active;
*
* \endinternal 
*******************************************************************************/
static  uint8 Launcher_GetActiveAppFromMetadata(void) CYSMALL \
...
{
    uint8 CYDATA result;
    uint8 CYDATA app0 = (uint8)Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE, 
                                                     Launcher_MD_BTLDB_ACTIVE_0);
    uint8 CYDATA app1 = (uint8)Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE, 
    ...
    if (0u != app0)                                                     
    {
        ...
        {
            /* app0 is active */
            result = Launcher_MD_BTLDB_ACTIVE_0;
        }
        else 
        {
            /* Both are active */
            result = Launcher_BOTH_ACTIVE;
        }
    }
    else /* (0u == app0) */
    {
        ...
        {
            /* app1 is active */
            result = Launcher_MD_BTLDB_ACTIVE_1;
        }
        else
        {
            /* Neither app is active. */
            result = Launcher_MD_BTLDB_ACTIVE_NONE;
        }          
    }
...
#if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
/*******************************************************************************
* Function Name: Launcher_Initialize
****************************************************************************//**
* 
* \brief 
*   Used for in-app bootloading. This function updates the global variable 
*   Launcher_runningApp with a running application number.
*
*   If the running application number is valid (0 or 1), this function also 
*   sets the global variable Launcher_initVar that is used to determine 
...
{
    if (Launcher_BOOTLOADING_NOT_INITIALIZED ==Launcher_initVar)
    {
        Launcher_activeApp = Launcher_GetActiveAppFromMetadata();
        
        /* Updating with number of active application */    
        if ((Launcher_MD_BTLDB_ACTIVE_NONE != Launcher_activeApp) &&
            (Launcher_BOTH_ACTIVE != Launcher_activeApp))
        {
            Launcher_runningApp = Launcher_activeApp;
            
            /* Bootloader commands are to be processed */
            Launcher_initVar = Launcher_BOOTLADING_INITIALIZED;
        }
        else 
        {
            Launcher_runningApp = Launcher_RUNNING_APPLICATION_UNKNOWN;
        }
    }
}

/*****************************************************************************
* Function Name: Launcher_GetRunningAppStatus
**************************************************************************//**
*
* \brief 
*   Used for dual-app or in-app bootloader. Returns the value of the global 
*   variable Launcher_runningApp. This function should be called only after the 
*   Launcher_Initialize() has been called once.
*
* \return 
...
{  
    return (Launcher_runningApp);
}


/*******************************************************************************
* Function Name: Launcher_GetActiveAppStatus
****************************************************************************//**
*
* \brief 
*   Used for dual-app or in-app bootloader. Returns the value of the global 
*   variable Launcher_activeApp. This function should be called only after the 
*   Launcher_Initialize() has been called once.
*
* \return 
...
{  
    ...
    return (Launcher_activeApp);
}
#endif /*(CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/

#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)
/*******************************************************************************
* Function Name: Launcher_CalcPacketChecksum
****************************************************************************//**
*  \internal
*
*  \brief
...
*  \endinternal
*******************************************************************************/
static uint16 Launcher_CalcPacketChecksum(const uint8 buffer[], uint16 size) \
                    CYSMALL 
{
    #if(0u != Launcher_PACKET_CHECKSUM_CRC)

        uint16 CYDATA crc = Launcher_CRC_CCITT_INITIAL_VALUE;
        uint16 CYDATA tmp;
        uint8  CYDATA i;
        ...
        else
        {
            do
            {
                ...
                for (i = 0u; i < 8u; i++)
                {
                    ...
                    {
                        crc = (crc >> 1u) ^ Launcher_CRC_CCITT_POLYNOMIAL;
                    }
                    else
                    {
                        crc >>= 1u;
    ...
    #endif /* (0u != Launcher_PACKET_CHECKSUM_CRC) */
}

#if (!CY_PSOC3)
/*******************************************************************************
* Function Name: Launcher_InitCallback
****************************************************************************//**
*
* \brief
...
void Launcher_InitCallback(Launcher_callback_type userCallback) \
           CYSMALL 
{
    Launcher_callback = userCallback;
}
#endif /* (!CY_PSOC3) */
...
#if (0u != Launcher_CMD_VERIFY_FLS_ROW_AVAIL)
/*******************************************************************************
* Function Name: Launcher_VerifyRow
****************************************************************************//**
*  \internal
* 
*  \brief
...
*  \n Launcher_ERR_VERIFY - If there is any mismatch.
*
* \endinternal 
*******************************************************************************/
static uint8 Launcher_VerifyRow(uint32 flashStart, const uint8* ramStart, uint16 size) \
...
/*******************************************************************************
* Function Name: Launcher_Calc8BitSum
****************************************************************************//**
*
* \brief
...
*
*******************************************************************************/
uint8 Launcher_Calc8BitSum(uint32 baseAddr, uint32 start, uint32 size) \
                CYSMALL 
{
    uint8 CYDATA sum = 0u;
...
#if ((0u != Launcher_COPIER_AVAIL) && (CY_PSOC4)) 
/*******************************************************************************
* Function Name: Launcher_CopyRow
****************************************************************************//**
* \internal
*
* \brief
                 ...
                 CYSMALL 
{
    ...
    {
        rowBuffer[idx] = Launcher_GET_CODE_BYTE(srcBaseAddr + idx);
    }
...
*    (Launcher_MAX_ATTEMPTS) to try again. In case of no success, 
           ...
           
{
    ...
    uint8 copyFlag = Launcher_GetMetadata(Launcher_GET_BTLDB_COPY_FLAG, Launcher_MD_BTLDB_ACTIVE_1);

    /* Buffer to temporary hold metadata before flashing */
    uint8 metaBuf[CYDEV_FLS_ROW_SIZE];

    /* Launcher's last row number */
    uint16 launcherLastRow = Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW, Launcher_MD_BTLDB_ACTIVE_0);
    ...
    uint32 srcLength = Launcher_GetMetadata(Launcher_GET_BTLDB_LENGTH, Launcher_MD_BTLDB_ACTIVE_1);
    ...
    * perform the copying operation (a number is defined by Launcher_MAX_ATTEMPTS)
    ***************************************************************************/
    for(idx = 0; idx < numberOfRows; idx++)
    {
        copierAttempts = Launcher_MAX_ATTEMPTS;
        
        do 
        {
            /* Copying row */
            copyStatus = Launcher_CopyRow(srcRow + idx, dstRow + idx);
    ...
    if (CYRET_SUCCESS == copyStatus)
    {
        ...
        ******************************************************************************/
        ...
        uint32 metaStartAddr = Launcher_MD_BASE_ADDR(Launcher_MD_BTLDB_ACTIVE_1) + 
                               Launcher_MD_SIZEOF - CYDEV_FLS_ROW_SIZE; 
        
        /* Read all data from metadata #2 */
        for (idx = 0u; idx < CYDEV_FLS_ROW_SIZE; idx++)
        {
            metaBuf[idx] = Launcher_GET_CODE_BYTE(metaStartAddr + idx);
        }
        
        /* Setting 'Copier available' flag (copyFlag field) in buffer to be stored in metadata#1 */
        metaBuf[Launcher_MD_BUFFER_START_OFFSET + Launcher_MD_COPY_FLAG_BYTE_OFFSET] = Launcher_COPIER_SUPPORT_SET_BIT;                

        /* Set active app#1 */
        Launcher_activeApp = Launcher_MD_BTLDB_ACTIVE_0;
        
        /* Setting app#1 as active application in buffer to be stored in metadata#1 */
        metaBuf[Launcher_MD_BUFFER_START_OFFSET + Launcher_MD_ACTIVE_APP_BYTE_OFFSET] = 0x01u;      

        /* Setting app#1 as valid for 'Fast validation' to be stored in metadata#1 */
        metaBuf[Launcher_MD_BUFFER_START_OFFSET + Launcher_MD_FAST_VALID_BYTE_OFFSET] = 0x01u;      
...
void Launcher_Start(void) CYSMALL 
{
    ...
    #if(0u != Launcher_BOOTLOADER_APP_VALIDATION)
        uint8 CYDATA calcedChecksum;
    #endif    /* (0u != Launcher_BOOTLOADER_APP_VALIDATION) */

    #if(!CY_PSOC4)
        #if(0u != Launcher_FAST_APP_VALIDATION)
            ...
            if (CYRET_SUCCESS != CySetTemp())
            {
                ...
                if (CYRET_SUCCESS != CySetFlashEEBuffer(Launcher_flashBuffer))
                {
                    CyHalt(0x00u);
                }

                uint8 CYXDATA Launcher_flashBuffer[Launcher_FROW_SIZE];
            #endif /* !defined(CY_BOOT_VERSION) */
        #endif  /* (0u != Launcher_FAST_APP_VALIDATION) */
    #endif  /* (!CY_PSOC4) */

    #if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER) 
    #if ((0u != Launcher_COPIER_AVAIL) && (CY_PSOC4))
        /* Checking "need-to-copy" flag in metadata#1. If it is set, then copy operation is required*/            
        uint8 needToCopyFlag = Launcher_GetMetadata(Launcher_GET_BTLDB_COPY_FLAG, 
                                                        Launcher_MD_BTLDB_ACTIVE_1);                                                    
        
        /* Checking "Need to copy" flag in metadata#1 */
        if (0u != (needToCopyFlag & Launcher_NEED_TO_COPY_MASK))
        {
    ...
    /* Identify active Bootloadable/Combination application */
    #if (0u != Launcher_DUAL_APP_BOOTLOADER)
        app0Status = Launcher_ValidateBootloadable(Launcher_MD_BTLDB_ACTIVE_0);
        app1Status = Launcher_ValidateBootloadable(Launcher_MD_BTLDB_ACTIVE_1);  

        /* Assumes no active Bootloadable application. Bootloader is active. */
        Launcher_activeApp = Launcher_MD_BTLDB_ACTIVE_NONE;

        /* Bootloadable app#1 is active */
        if(Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE, Launcher_MD_BTLDB_ACTIVE_0) == \
                                                                           Launcher_MD_BTLDB_IS_ACTIVE)
        {
            /**********************************************************************************************************
            * ---------------------------------------------------------------------------------------------------------
            ...
            **********************************************************************************************************/
            ...
            {
                #if (((1u == Launcher_AUTO_SWITCHING_AVAIL) &&  \
                      (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_MULTIAPPBOOTLOADER)) || \
                      (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER)) 
                if (CYRET_SUCCESS == app1Status )
                {
                    /* Cases # 10 and 12 */
                    Launcher_activeApp = Launcher_MD_BTLDB_ACTIVE_1;
                    validApp = CYRET_SUCCESS;
                }
                #endif /*((1u == Launcher_AUTO_SWITCHING_AVAIL) && 
                          (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_MULTIAPPBOOTLOADER)) || 
                          (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LAUNCHER)*/
            }
        }
        ...
        if(Launcher_activeApp == Launcher_MD_BTLDB_ACTIVE_NONE)
        {
            /**********************************************************************************************************
            * ---------------------------------------------------------------------------------------------------------
            * |      |       app#1    |       app#2    |        Classic Dual-app       |     Launch-only Bootloader   |
            ...
            * ---------------------------------------------------------------------------------------------------------
            **********************************************************************************************************/
            if (Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE, 1u) ==
                    Launcher_MD_BTLDB_IS_ACTIVE)
            {
                /* Cases # 3, 4, 7, and 8 */
                if (CYRET_SUCCESS == app1Status)
        ...
        if (CYRET_SUCCESS == Launcher_ValidateBootloadable(Launcher_MD_BTLDB_ACTIVE_0))
        {
            validApp = CYRET_SUCCESS;
        }
    #endif  /* (0u != Launcher_DUAL_APP_BOOTLOADER) */
    ...
    ***********************************************************************/
        ...
        calcedChecksum = Launcher_Calc8BitSum(CY_FLASH_BASE,
                Launcher_MD_BTLDR_ADDR_PTR,
                *Launcher_SizeBytesAccess - Launcher_MD_BTLDR_ADDR_PTR);

        /* We included checksum, so remove it */
        calcedChecksum -= *Launcher_ChecksumAccess;
        calcedChecksum = ( uint8 )1u + ( uint8 )(~calcedChecksum);

        /* Checksum and pointer to Bootloader verification */
        if((calcedChecksum != *Launcher_ChecksumAccess) ||
           (0u == *Launcher_SizeBytesAccess))
        {
            /***********************************************************************
            * If the Bootloader/Launcher application calculated checksum does not match
            * the checksum stored in special Launcher_Checksum constant 
            * (accessed through Launcher_ChecksumAccess) constant or its size 
            * stored in Launcher_SizeBytes constant (accessed through
            * Launcher_SizeBytesAccess) then CPU is halted, all software 
            * processing stops. 
            ***********************************************************************/
            CyHalt(0x00u);
        }
    ...
    *    commands forever.
    ***********************************************************************/
    if ((Launcher_GET_RUN_TYPE == Launcher_SCHEDULE_BTLDR) 
#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)        
        || (CYRET_SUCCESS != validApp)
#endif        
        )
    {
        Launcher_SET_RUN_TYPE(Launcher_SCHEDULE_BTLDR_INIT_STATE);

        Launcher_HostLink(Launcher_WAIT_FOR_COMMAND_FOREVER);
    }

    /* Go to communication subroutine. Will wait for commands for specified time */
    #if(0u != Launcher_WAIT_FOR_COMMAND)

        /* Timeout is in 100s of milliseconds */
        Launcher_HostLink(Launcher_WAIT_FOR_COMMAND_TIME);

    #endif  /* (0u != Launcher_WAIT_FOR_COMMAND) */
#else /* Launcher */
    ...
    Launcher_LaunchApplication();
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/    
}

#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
/*******************************************************************************
* Function Name: Launcher_LaunchApplication
****************************************************************************//**
* \internal
*
* \brief
...
*   - Launcher_EXIT_TO_BTLDR - The Bootloader application will be started on
*                                     a software reset.
*   - Launcher_EXIT_TO_BTLDB;
*   - Launcher_EXIT_TO_BTLDB_1 - Bootloadable application # 1 will be
*                                     started on a software reset.
*   - Launcher_EXIT_TO_BTLDB_2 - Bootloadable application # 2 will be
...
{
    if(Launcher_EXIT_TO_BTLDR == appId)
    {
        Launcher_SET_RUN_TYPE(Launcher_SCHEDULE_BTLDR_INIT_STATE);
    }
    else
    {
        if(CYRET_SUCCESS == Launcher_ValidateBootloadable(appId))
        {
            /* Set active application in metadata */
            uint8 CYDATA idx;
            for(idx = 0u; idx < Launcher_MAX_NUM_OF_BTLDB; idx++)
            {
                Launcher_SetFlashByte((uint32) Launcher_MD_BTLDB_ACTIVE_OFFSET(idx),
                                              (uint8 )(idx == appId));
            }

        #if((0u != Launcher_DUAL_APP_BOOTLOADER) || \
            (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))
            Launcher_activeApp = appId;
        #endif /* (0u != Launcher_DUAL_APP_BOOTLOADER) || 
                  (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))*/
...
void CyBtldr_CheckLaunch(void) CYSMALL 
{
    ...
    *******************************************************************************/
    ...
    if (Launcher_GET_RUN_TYPE == Launcher_SCHEDULE_BTLDB)
    {
        ...
        * The active application number is taken from Launcher_activeApp not
        * to break backward compatibility.
        *******************************************************************************/
        uint8 activeApp = Launcher_activeApp;
        ...
        * between applications. So if there is no valid value for Launcher_activeApp,
        ...
        *******************************************************************************/ 
        ...
        if (activeApp >= Launcher_MD_BTLDB_ACTIVE_NONE) 
        {
            activeApp = Launcher_GetActiveAppFromMetadata();
        }
        ...
        uint8 activeApp = Launcher_GetActiveAppFromMetadata(); 
    #endif /* (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER) */
        ...
        *******************************************************************************/
        if ((0u != Launcher_GetMetadata(Launcher_GET_BTLDB_ADDR, activeApp)) 
    #if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_BOOTLOADER)
            && (activeApp < Launcher_MD_BTLDB_ACTIVE_NONE)
    #endif /* (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_BOOTLOADER) */        
            )
        {
            /* Never returns from this method */
            Launcher_LaunchBootloadable(Launcher_GetMetadata(Launcher_GET_BTLDB_ADDR,
                                                                             activeApp));
        }
    }
}
...
/* Moves argument appAddr (RO) into PC, moving execution to appAddr */
    ...
    __asm static void Launcher_LaunchBootloadable(uint32 appAddr)
    {
        BX  R0
        ALIGN
    }
...
/*******************************************************************************
* Function Name: Launcher_ValidateBootloadable
****************************************************************************//**
* \brief
...
*******************************************************************************/
cystatus Launcher_ValidateBootloadable(uint8 appId) CYSMALL \

    {
        uint32 CYDATA idx;

        uint32 CYDATA end   = Launcher_FIRST_APP_BYTE(appId) +
                              Launcher_GetMetadata(Launcher_GET_BTLDB_LENGTH, appId);
        ...
        #if(0u != Launcher_FAST_APP_VALIDATION)
            if(Launcher_GetMetadata(Launcher_GET_BTLDB_STATUS, appId) ==
               Launcher_MD_BTLDB_IS_VERIFIED)
            {
        ...
        for(idx = Launcher_FIRST_APP_BYTE(appId); idx < end; ++idx)
        {
            uint8 CYDATA curByte = Launcher_GET_CODE_BYTE(idx);
        ...
        ***************************************************************************/
            ...
            idx = ((Launcher_FIRST_APP_BYTE(appId)) >> 3u);

            /* Flash may run into meta data, so ECC does not use full row */
            end = (end == (CY_FLASH_SIZE - Launcher_MD_SIZEOF))
                ? (CY_FLASH_SIZE >> 3u)
                : (end >> 3u);
        ...
        if((calcedChecksum != Launcher_GetMetadata(Launcher_GET_BTLDB_CHECKSUM, appId)) ||
           (0u == valid))
        {
            return(CYRET_BAD_DATA);
        }
        ...
        #if(0u != Launcher_FAST_APP_VALIDATION)
            Launcher_SetFlashByte((uint32) Launcher_MD_BTLDB_VERIFIED_OFFSET(appId),
                                          Launcher_MD_BTLDB_IS_VERIFIED);
        #endif  /* (0u != Launcher_FAST_APP_VALIDATION) */
        ...
        return(CYRET_SUCCESS);
                                          ...
                                          || (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))
/*******************************************************************************
* Function Name: Launcher_CheckImage
********************************************************************************
* \internal
*
* \brief 
...
*   Launcher_ERR_ACTIVE - Writing to this image is NOT permitted
                                         ...
                                         
{
    ...
    if (appNumber < Launcher_MAX_NUM_OF_BTLDB)
    {    
        /*******************************************************************************
        ...
        *******************************************************************************/
        firstRow = (uint16) 1u +
            (uint16) Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW,  appNumber);
    ...
    ***********************************************************************************/
    ...
    #if ((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) && \
         (0u != Launcher_IS_STACK_APPLICATION))
        /*******************************************************************************
        ...
        *                |<-------------------lastRow -------------------->|
        *******************************************************************************/
        lastRow = (uint16) Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW,
                                                        Launcher_USER_APPLICATION);    
        ...
        *                                         |<-------lastRow-------->|
        *******************************************************************************/
        lastRow = (uint16)(CY_FLASH_NUMBER_ROWS - Launcher_NUMBER_OF_METADATA_ROWS - firstRow);
        ...
        *******************************************************************************/
        lastRow = (uint16)(lastRow / (uint8)(Launcher_NUMBER_OF_BTLDBLE_APPS - appNumber));
    ...
    #endif /*(0u == Launcher_IS_STACK_APPLICATION)*/    
        ...
        *
        *******************************************************************************/
        if (((row >= firstRow) && (row <= lastRow)) || ((arrayNumber == Launcher_MD_FLASH_ARRAY_NUM) && \
            (rowNumInArray == Launcher_MD_ROW_NUM(appNumber))))
        {
            ackCode = Launcher_ERR_ACTIVE;
        }
    }
    else /*(appNumber < Launcher_MAX_NUM_OF_BTLDB)*/
    {
        ackCode = Launcher_ERR_ACTIVE;
    }
...
#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)
/*******************************************************************************
* Function Name: Launcher_HostLink
****************************************************************************//**
*
* \brief
...
#if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
    static  CYBIT communicationState = Launcher_COMMUNICATION_STATE_IDLE; 
    static uint16 CYDATA dataOffset = 0u; 
    uint16  CYDATA btldrLastRow = 0xFFFFu;
    uint8 needToCopyFlag = 0u;
    uint32 app2StartAddress = Launcher_GetMetadata(Launcher_GET_BTLDB_ADDR, 
                              Launcher_MD_BTLDB_ACTIVE_1);
#else
    CYBIT     communicationState = Launcher_COMMUNICATION_STATE_IDLE;
    uint16 CYDATA dataOffset = 0u; 
#endif /*CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER*/

    uint8     packetBuffer[Launcher_SIZEOF_COMMAND_BUFFER];
    uint8     dataBuffer  [Launcher_SIZEOF_COMMAND_BUFFER];
    ...
    do
    {
        ...
        do
        {
            readStat = CyBtldrCommRead(packetBuffer,
                                        Launcher_SIZEOF_COMMAND_BUFFER,
                                        &numberRead,
        ...
        if((numberRead < Launcher_MIN_PKT_SIZE) ||
           (packetBuffer[Launcher_SOP_ADDR] != Launcher_SOP))
        {
            ackCode = Launcher_ERR_DATA;
        }
        else
        {
            pktSize = ((uint16)((uint16)packetBuffer[Launcher_SIZE_ADDR + 1u] << 8u)) |
                               packetBuffer[Launcher_SIZE_ADDR];
            ...
            **************************************************************************************/
            if (((pktSize + Launcher_MIN_PKT_SIZE) > numberRead) || 
                ((pktSize + Launcher_MIN_PKT_SIZE) > Launcher_SIZEOF_COMMAND_BUFFER)) 
            {
                ackCode = Launcher_ERR_LENGTH;
            }
            else /* Packet length is OK*/
            {
                pktChecksum = ((uint16)((uint16)packetBuffer[Launcher_CHK_ADDR(pktSize) + 1u] << 8u)) |
                                   packetBuffer[Launcher_CHK_ADDR(pktSize)];
   
                if (packetBuffer[Launcher_EOP_ADDR(pktSize)] != Launcher_EOP)
                {
                    ackCode = Launcher_ERR_DATA;
                }
                else if(pktChecksum != Launcher_CalcPacketChecksum(packetBuffer,
                                                                        pktSize + Launcher_DATA_ADDR))
                {
                    ackCode = Launcher_ERR_CHECKSUM;
                }
                else
                {
                    /* Empty section */
        ...
        rspSize = Launcher_RSP_SIZE_0;
        if(ackCode == CYRET_SUCCESS)
        {
            uint8 CYDATA btldrData = packetBuffer[Launcher_DATA_ADDR];

            ackCode = Launcher_ERR_DATA;
            switch(packetBuffer[Launcher_CMD_ADDR])
            {
            ...
            *   Get metadata (0x3C)
            ***************************************************************************/
            #if(0u != Launcher_CMD_GET_METADATA)

                case Launcher_COMMAND_GET_METADATA:

                    if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        if (btldrData >= Launcher_MAX_NUM_OF_BTLDB)
                        {
                            ackCode = Launcher_ERR_APP;
                        }
                        else if(CYRET_SUCCESS == Launcher_ValidateBootloadable(btldrData))
                        {
                            #if(CY_PSOC3)
                                (void) memcpy(&packetBuffer[Launcher_DATA_ADDR],
                                            ((uint8  CYCODE *) (Launcher_MD_BASE_ADDR(btldrData))),
                                            Launcher_GET_METADATA_RESPONSE_SIZE);
                            #else
                                (void) memcpy(&packetBuffer[Launcher_DATA_ADDR],
                                            (uint8 *) Launcher_MD_BASE_ADDR(btldrData),
                                            Launcher_GET_METADATA_RESPONSE_SIZE);
                            #endif  /* (CY_PSOC3) */

                            rspSize = Launcher_RSP_SIZE_GET_METADATA;
                            ackCode = CYRET_SUCCESS;
                        }
                        else
                        {
            ...
            #endif  /* (0u != Launcher_CMD_GET_METADATA) */


            /***************************************************************************
            *   Verify application checksum (0x31)
            ***************************************************************************/
        #if (0u != Launcher_CMD_VERIFY_APP_CHKSUM_AVAIL)     
            case Launcher_COMMAND_CHECKSUM:

                if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 0u))
                {
                    packetBuffer[Launcher_DATA_ADDR] =
                            (uint8)(Launcher_ValidateBootloadable(Launcher_activeApp) == CYRET_SUCCESS);

                    rspSize = Launcher_RSP_SIZE_VERIFY_CHKSM;
                    ackCode = CYRET_SUCCESS;
                }
                break;
        #endif /* (0u != Launcher_CMD_VERIFY_APP_CHKSUM_AVAIL) */

            /*****************************************************************************
            *   Verify row (0x45u)
            ***************************************************************************/
        #if(0u != Launcher_CMD_VERIFY_FLS_ROW_AVAIL)
            case Launcher_COMMAND_VERIFY_FLS_ROW:

                /* Packet size is either 3 (data is already in buffer) or (3u + Launcher_FROW_SIZE),
                 * then data is in packetBuffer[]*/
                if ((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && ((3u == pktSize) || 
                   ((3u + Launcher_FROW_SIZE) == pktSize)))
                {
                    uint16 CYDATA rowNum = ((uint16)((uint16)packetBuffer[Launcher_DATA_ADDR + 2u] << 8u)) |
                                                             packetBuffer[Launcher_DATA_ADDR + 1u];
                    ...
                    else 
                    {
                        ...
                        (void) memcpy(dataBuffer, &packetBuffer[Launcher_DATA_ADDR + 3u],
                                      (int16)pktSize - 3);
                    #else              
                        /*Data is in packetBuffer[], needs to be copied to dataBuffer[]*/
                        (void) memcpy(dataBuffer, &packetBuffer[Launcher_DATA_ADDR + 3u],
                ...
                #if (!CY_PSOC4) 
                    if((btldrData >= Launcher_FIRST_EE_ARRAYID) &&
                       (btldrData <= Launcher_LAST_EE_ARRAYID))
                    {
                        /* EEPROM */
                        ...
                        upperRange = Launcher_NUMBER_OF_EEPROM_SECTORS * Launcher_NUMBER_OF_ROWS_IN_EEPROM_SECTOR;
                    }
                    else if (btldrData < CY_FLASH_NUMBER_ARRAYS)    
                    {
                        ...
                        upperRange = Launcher_NUMBER_OF_ROWS_IN_ARRAY;                        
                    }
                    else
                    {
                        ackCode = Launcher_ERR_ARRAY;
                        break;                              
                    }

                    /*Checking if row number is within array address range*/
                    ackCode = Launcher_CHECK_ROW_NUMBER(rowNum, upperRange);
                    ...
                    ackCode = Launcher_VerifyRow(startAddr, dataBuffer, rowSize);
                    ...
                    *******************************************************************************/                    
                        ...
                        if ((CYRET_SUCCESS == ackCode) && (btldrData < CY_FLASH_NUMBER_ARRAYS) && (dataOffset > rowSize))
                        {
                            ...
                            ackCode = Launcher_VerifyRow(startAddr, &dataBuffer[rowSize], (uint16)CYDEV_ECC_ROW_SIZE);                        
                        }
                    ...
                    if (btldrData < CY_FLASH_NUMBER_ARRAYS)
                    {
                        ...
                        ackCode = Launcher_VerifyRow(startAddr, dataBuffer, (uint16)CYDEV_FLS_ROW_SIZE);
                    }
                    else
                    {
                        ackCode = Launcher_ERR_ARRAY;
                ...
                break;
        #endif /*(0u != Launcher_CMD_VERIFY_ROW_FLS_AVAIL)*/

            /***************************************************************************
            *   Get flash size (0x32)
            ***************************************************************************/
            #if(0u != Launcher_CMD_GET_FLASH_SIZE_AVAIL)

                case Launcher_COMMAND_REPORT_SIZE:
                    ...
                    if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        if(btldrData < CY_FLASH_NUMBER_ARRAYS)
                        {
                            ...
                            *   then we do not use the Launcher_SizeBytes and Launcher_SizeBytesAccess 
                            *   variables, instead we take the Launcher's last row from metadata.
                            *******************************************************************************/
                        #if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) 
                            arrayIdBtlderEnds = (uint8) (*Launcher_SizeBytesAccess / CY_FLASH_SIZEOF_ARRAY);

                            if (btldrData == arrayIdBtlderEnds)
                            {
                                startRow = (uint16) (*Launcher_SizeBytesAccess / CY_FLASH_SIZEOF_ROW) %
                                            Launcher_NUMBER_OF_ROWS_IN_ARRAY;
                            }
                        #else /*Combination project type*/   
                            btldrLastRow = (uint16)(Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW, \
                                                                                Launcher_MD_BTLDB_ACTIVE_0));
                            arrayIdBtlderEnds = (uint8) (((uint32)btldrLastRow * CY_FLASH_SIZEOF_ROW) / (uint32)CY_FLASH_SIZEOF_ARRAY); 
                            if (btldrData == arrayIdBtlderEnds)
                            {
                                startRow = (uint16)((btldrLastRow) % Launcher_NUMBER_OF_ROWS_IN_ARRAY);
                            }
                        #endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/
                        
                            else if (btldrData > arrayIdBtlderEnds)
                            {
                                startRow = Launcher_FIRST_ROW_IN_ARRAY;
                            }
                            else /* (btldrData < ArrayIdBtlderEnds) */
                            {
                                startRow = Launcher_NUMBER_OF_ROWS_IN_ARRAY;
                            }

                            packetBuffer[Launcher_DATA_ADDR]      = LO8(startRow);
                            packetBuffer[Launcher_DATA_ADDR + 1u] = HI8(startRow);

                            packetBuffer[Launcher_DATA_ADDR + 2u] =
                                        LO8(Launcher_NUMBER_OF_ROWS_IN_ARRAY - 1u);

                            packetBuffer[Launcher_DATA_ADDR + 3u] =
                                        HI8(Launcher_NUMBER_OF_ROWS_IN_ARRAY - 1u);

                            rspSize = Launcher_RSP_SIZE_GET_FLASH_SIZE;
                            ackCode = CYRET_SUCCESS;
                        }
            ...
            #endif  /* (0u != Launcher_CMD_GET_FLASH_SIZE_AVAIL) */
            ...
            ***************************************************************************/
                ...
                #if(0u != Launcher_CMD_GET_APP_STATUS_AVAIL)

                    case Launcher_COMMAND_APP_STATUS:

                        if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (1u == pktSize))
                        {

                            packetBuffer[Launcher_DATA_ADDR] =
                                (uint8)Launcher_ValidateBootloadable(btldrData);

                            packetBuffer[Launcher_DATA_ADDR + 1u] =
                                (uint8) Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE, btldrData);

                            rspSize = Launcher_RSP_SIZE_GET_APP_STATUS;
                            ackCode = CYRET_SUCCESS;
                        }
                        break;

                #endif  /* (0u != Launcher_CMD_GET_APP_STATUS_AVAIL) */
            ...
            ***************************************************************************/
            case Launcher_COMMAND_PROGRAM:

            /* btldrData variable holds Flash Array ID */

        #if (0u != Launcher_CMD_ERASE_ROW_AVAIL)

            case Launcher_COMMAND_ERASE:
                if (Launcher_COMMAND_ERASE == packetBuffer[Launcher_CMD_ADDR])
                {
                    if ((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 3u))
                    {
                        #if(!CY_PSOC4)
                            if((btldrData >= Launcher_FIRST_EE_ARRAYID) &&
                               (btldrData <= Launcher_LAST_EE_ARRAYID))
        ...
        #endif  /* (0u != Launcher_CMD_ERASE_ROW_AVAIL) */
                ...
                if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize >= 3u))
                {
                    ...
                    if(dataOffset == pktSize)
                    {
                        ...
                        uint16 firstRow;    
                    #if ((0u != Launcher_DUAL_APP_BOOTLOADER) && \
                          (1u == Launcher_GOLDEN_IMAGE_AVAIL))
                        cystatus goldenImageValid;
                    #endif /*(((0u != Launcher_DUAL_APP_BOOTLOADER) &&
                               (1u == Launcher_GOLDEN_IMAGE_AVAIL))*/

                        /* Get flash/EEPROM row number inside array */
                        dataOffset = ((uint16)((uint16)packetBuffer[Launcher_DATA_ADDR + 2u] << 8u)) |
                                              packetBuffer[Launcher_DATA_ADDR + 1u];

                        /*Checking if row number is within array address range*/
                        ackCode = Launcher_CHECK_ROW_NUMBER(dataOffset, upperRange);
                        ...
                        #if(!CY_PSOC4)
                            if(btldrData <= Launcher_LAST_FLASH_ARRAYID)
                            {
                        #endif  /* (!CY_PSOC4) */
                
                        /* btldrData  - holds flash array Id sent by host */
                        /* dataOffset - holds flash row Id sent by host   */
                        row = (uint16)(btldrData * Launcher_NUMBER_OF_ROWS_IN_ARRAY) + dataOffset;
                        ...
                        *******************************************************************************/
                        ...
                        /* First empty flash row after Bootloader application */
                        #if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)    
                            firstRow = (uint16) (*Launcher_SizeBytesAccess / CYDEV_FLS_ROW_SIZE);
                            if ((*Launcher_SizeBytesAccess % CYDEV_FLS_ROW_SIZE) != 0u)
                            {
                                firstRow++;
                            }
                        #else /*Combination project type*/                       
                            
                            if (Launcher_RUNNING_APPLICATION_0 == Launcher_runningApp) 
                            {
                                btldrLastRow = (uint16)Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW, 
                                                                            Launcher_MD_BTLDB_ACTIVE_1);                            
                            }    
                            else /*(Launcher_RUNNING_APPLICATION_1 == Launcher_runningApp)*/
                            {
                                btldrLastRow = (uint16)Launcher_GetMetadata(Launcher_GET_BTLDR_LAST_ROW,
                                                                            Launcher_MD_BTLDB_ACTIVE_0);
                            }
                            ...
                            ************************************************************************************/
                            ...
                            if (0u != app2StartAddress) 
                            {
                                ...
                                if(Launcher_activeApp < Launcher_MD_BTLDB_ACTIVE_NONE)    
                                {
                                    ...
                                    ackCode = Launcher_CheckImage(Launcher_activeApp, 
                                                                       btldrData, /*array number*/
                                                                       row,  /*row number*/
                                                                       dataOffset); /*row number in scope of array*/
                                
                                    if (Launcher_ERR_ACTIVE == ackCode)
                                    {
                                        dataOffset = 0u;
                                        break;                                
                                    }
                            ...
                            ackCode = Launcher_CheckImage(Launcher_GOLDEN_IMAGE, 
                                                                  btldrData, /*array number*/
                                                                  row, /*row number*/
                                                                  dataOffset); /*row number in scope of array*/
                            ...
                            *************************************************************************/
                            goldenImageValid = Launcher_ValidateBootloadable(Launcher_GOLDEN_IMAGE);                          
                            if ((Launcher_ERR_ACTIVE == ackCode) && (CYRET_SUCCESS == goldenImageValid))
                            {
                                dataOffset = 0u;
                                break;                                
                            }
                        ...
                        *******************************************************************************/
                            ...
                            if(0u == clearedMetaData)
                            {
                                ...
                                uint8 erase[Launcher_FROW_SIZE];
                                uint8 Launcher_notActiveApp;


                                #if(CY_PSOC3)
                                    (void) memset(erase, (char8) 0, (int16) Launcher_FROW_SIZE);
                                #else
                                    (void) memset(erase, 0, Launcher_FROW_SIZE);
                                #endif  /* (CY_PSOC3) */


                                #if ((0u != Launcher_DUAL_APP_BOOTLOADER) || \
                                     (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))
                                    if (Launcher_MD_BTLDB_ACTIVE_0 == Launcher_activeApp)
                                    {
                                        Launcher_notActiveApp = Launcher_MD_BTLDB_ACTIVE_1;
                                    }
                                    else
                                    {
                                        Launcher_notActiveApp = Launcher_MD_BTLDB_ACTIVE_0;
                                    }
                                #else
                                    Launcher_notActiveApp = Launcher_MD_BTLDB_ACTIVE_0;
                                #endif /* (0u != Launcher_DUAL_APP_BOOTLOADER) */
                                    ...
                                    (void) CySysFlashWriteRow(
                                            Launcher_MD_ROW_NUM(Launcher_notActiveApp),
                                            erase);
                                #else
                                    (void) CyWriteRowFull(
                                            (uint8)  Launcher_MD_FLASH_ARRAY_NUM,
                                            (uint16) Launcher_MD_ROW_NUM(Launcher_notActiveApp),
                                            erase,
                                            Launcher_FROW_SIZE);
                                #endif  /* (CY_PSOC4) */
                        ...
                        #if(!CY_PSOC4)
                            }   /* (btldrData <= Launcher_LAST_FLASH_ARRAYID) */
                        #endif  /* (!CY_PSOC4) */
            ...
            ***************************************************************************/
            #if(0u != Launcher_CMD_SYNC_BOOTLOADER_AVAIL)

            case Launcher_COMMAND_SYNC:

                if(Launcher_COMMUNICATION_STATE_ACTIVE == communicationState)
                {
            ...
            #endif  /* (0u != Launcher_CMD_SYNC_BOOTLOADER_AVAIL) */
            ...
            ***************************************************************************/
                ...
                case Launcher_COMMAND_APP_ACTIVE:

                    if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        if(CYRET_SUCCESS == Launcher_ValidateBootloadable(btldrData))
                        {
                            Launcher_SetActiveAppInMetadata(btldrData);
                            Launcher_activeApp = btldrData;
                            ackCode = CYRET_SUCCESS;
                        }
                        else
                        {
            ...
            *   Send data (0x37)
            ***************************************************************************/
            #if (0u != Launcher_CMD_SEND_DATA_AVAIL)

                case Launcher_COMMAND_DATA:

                    if(Launcher_COMMUNICATION_STATE_ACTIVE == communicationState)
                    {
                        /*  Make sure that dataOffset is valid before copying data */
                        if((dataOffset + pktSize) <= Launcher_SIZEOF_COMMAND_BUFFER)
                        {
                            ackCode = CYRET_SUCCESS;
            ...
            #endif  /* (0u != Launcher_CMD_SEND_DATA_AVAIL) */


            /***************************************************************************
            *   Enter bootloader (0x38)
            ***************************************************************************/
            case Launcher_COMMAND_ENTER:

        #if (Launcher_SECURITY_KEY_AVAIL == 1u)    
            if(pktSize == 6u) /*Security key*/
        #else    
            if(pktSize == 0u) /*Security key is not demanded*/
        #endif /*(Launcher_SECURITY_KEY_AVAIL == 1u)*/   
                {
                    #if(CY_PSOC3)

                        Launcher_ENTER CYDATA BtldrVersion =
                            {CYSWAP_ENDIAN32(CYDEV_CHIP_JTAG_ID), CYDEV_CHIP_REV_EXPECT, Launcher_VERSION};

                    #else

                        Launcher_ENTER CYDATA BtldrVersion =
                            {CYDEV_CHIP_JTAG_ID, CYDEV_CHIP_REV_EXPECT, Launcher_VERSION};
            #endif  /* (CY_PSOC3) */

            #if (Launcher_SECURITY_KEY_AVAIL == 1u)
                
                /*Security key checking*/
                uint8 idx = Launcher_SECURITY_KEY_LENGTH;
                ackCode = CYRET_SUCCESS;
                
                do
                {
                    idx--;                     
                    if (Launcher_securityKey[idx] != packetBuffer[Launcher_DATA_ADDR + idx])
                    {
                        ackCode = Launcher_ERR_DATA;
                        break;
                    }
                ...
                #if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) 
                    Launcher_isBootloading = Launcher_BOOTLOADING_IN_PROGRESS;
                #endif /*(CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/    
                
                    communicationState = Launcher_COMMUNICATION_STATE_ACTIVE;

                    rspSize = sizeof(Launcher_ENTER);
                    dataOffset = 0u; 
            ...
            ***************************************************************************/
            #if (0u != Launcher_CMD_GET_ROW_CHKSUM_AVAIL)

            case Launcher_COMMAND_GET_ROW_CHKSUM:
                ...
                if((Launcher_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 3u))
                {
                    ...
                    uint16 CYDATA tmpRowNum; 
                           ...
                           (btldrData <= Launcher_LAST_EE_ARRAYID))
                        {
                            ...
                            /* Checking if row number is within array address range */
                            ackCode = Launcher_CHECK_ROW_NUMBER(rowNum, 
                                (Launcher_NUMBER_OF_EEPROM_SECTORS * Launcher_NUMBER_OF_ROWS_IN_EEPROM_SECTOR));
                            ...
                            checksum = Launcher_Calc8BitSum(CY_EEPROM_BASE, rowAddr, CYDEV_EEPROM_ROW_SIZE);
                        }
                        else
                        {
                            /* flash */ 
                            ...
                            checksum = Launcher_Calc8BitSum(CY_FLASH_BASE, rowAddr, CYDEV_FLS_ROW_SIZE);
                        }
                    ...
                    * correct.
                     ******************************************************************************/
                        ...
                        tmpRowNum = rowNum; 
                    #else
                        tmpRowNum = rowNum + ((uint16)(Launcher_NUMBER_OF_ROWS_IN_ARRAY * btldrData));
                    #endif
                    
                    if((Launcher_MD_FLASH_ARRAY_NUM == btldrData) && (Launcher_CONTAIN_METADATA(tmpRowNum)))
                    {

                        checksum -= (uint8)Launcher_GetMetadata(Launcher_GET_BTLDB_ACTIVE,
                                                                 Launcher_GET_APP_ID(tmpRowNum));

                        checksum -= (uint8)Launcher_GetMetadata(Launcher_GET_BTLDB_STATUS,
                                                                 Launcher_GET_APP_ID(tmpRowNum));
                    }

                    packetBuffer[Launcher_DATA_ADDR] = (uint8)1u + (uint8)(~checksum);
                    ackCode = CYRET_SUCCESS;
                    rspSize = Launcher_RSP_SIZE_VERIFY_ROW_CHKSM;
                }
                break;

            #endif /* (0u != Launcher_CMD_GET_ROW_CHKSUM_AVAIL) */


            /***************************************************************************
            *   Exit bootloader (0x3B)
            ***************************************************************************/
            case Launcher_COMMAND_EXIT:
                ...
                *******************************************************************************/
            #if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
                needToCopyFlag = Launcher_GetMetadata(Launcher_GET_BTLDB_COPY_FLAG, 
                                                        Launcher_MD_BTLDB_ACTIVE_1);                                                    
                ...
                if(CYRET_SUCCESS == Launcher_ValidateBootloadable(Launcher_activeApp))
                {
                    Launcher_SET_RUN_TYPE(Launcher_SCHEDULE_BTLDB);
                }
                
            #if (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)
                Launcher_isBootloading = Launcher_BOOTLOADING_COMPLETED;
            #endif /* CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER */    
            ...
            ***************************************************************************/
            ...
            #if (!CY_PSOC3)
                if ((NULL != Launcher_callback) && (Launcher_COMMUNICATION_STATE_ACTIVE == communicationState))
                {
                    Launcher_in_packet_type Launcher_inPacket; 
                    Launcher_out_packet_type Launcher_outPacket;

                    /* Initializing input packet */
                    Launcher_inPacket.command = packetBuffer[Launcher_CMD_ADDR];
                    Launcher_inPacket.packetLength = pktSize;
                    Launcher_inPacket.pInputBuffer = &packetBuffer[Launcher_DATA_ADDR];
                                        
                    /* Clearing and setting output packet */
                    (void) memset(&Launcher_outPacket, 0, sizeof(Launcher_outPacket));
                    Launcher_outPacket.pOutputBuffer = &packetBuffer[Launcher_DATA_ADDR];
                    
                    /* Callback */
                    (void) Launcher_callback(&Launcher_inPacket, &Launcher_outPacket);

                    rspSize = Launcher_outPacket.packetLength;
                    if (((rspSize > (Launcher_SIZEOF_COMMAND_BUFFER - Launcher_MIN_PKT_SIZE)) && 
                         (Launcher_outPacket.pOutputBuffer == &packetBuffer[Launcher_DATA_ADDR])) ||
                        ((NULL == Launcher_outPacket.pOutputBuffer) && (0u != rspSize)))
                    {                    
                        /* One of returned parameters is wrong */
                        ackCode = Launcher_ERR_CALLBACK;
                        rspSize = 0u;
                    }
                    else /* Everything is OK */
                    {
                        ackCode = (uint8)Launcher_outPacket.status;
                    }
                }    
                else /* No callback function defined, so return "Error Command" status */
            #endif /* (!CY_PSOC3) */
                {
                    ackCode = Launcher_ERR_CMD;
                }    
                break;
            }
        }
        ...
        (void) Launcher_WritePacket(ackCode, packetBuffer, rspSize);
   
    } while ((0u == timeOut) || (Launcher_COMMUNICATION_STATE_ACTIVE == communicationState));
}


/*******************************************************************************
* Function Name: Launcher_WritePacket
****************************************************************************//**
* \internal
*
* \brief
                                            ...
                                            
{
    ...
    /* Start of packet. */
    buffer[Launcher_SOP_ADDR]      = Launcher_SOP;
    buffer[Launcher_CMD_ADDR]      = status;
    buffer[Launcher_SIZE_ADDR]     = LO8(size);
    buffer[Launcher_SIZE_ADDR + 1u] = HI8(size);

    /* Compute checksum. */
    checksum = Launcher_CalcPacketChecksum(buffer, size + Launcher_DATA_ADDR);

    buffer[Launcher_CHK_ADDR(size)]     = LO8(checksum);
    buffer[Launcher_CHK_ADDR(1u + size)] = HI8(checksum);
    buffer[Launcher_EOP_ADDR(size)]     = Launcher_EOP;

    /* Start packet transmit. */
    return(CyBtldrCommWrite(buffer, size + Launcher_MIN_PKT_SIZE, &size, 150u));
}
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)*/

/*******************************************************************************
* Function Name: Launcher_SetFlashByte
****************************************************************************//**
*
* \brief
...
void Launcher_SetFlashByte(uint32 address, uint8 runType) 
{
    ...
    for(idx = 0u; idx < CYDEV_FLS_ROW_SIZE; idx++)
    {
        rowData[idx] = Launcher_GET_CODE_BYTE(baseAddr + idx);
    }
...
/*******************************************************************************
* Function Name: Launcher_GetMetadata
****************************************************************************//**
*
* \brief
...
uint32 Launcher_GetMetadata(uint8 field, uint8 appId)
{
    ...
    switch (field)
    {
    case Launcher_GET_BTLDB_CHECKSUM:
        fieldPtr  = Launcher_MD_BTLDB_CHECKSUM_OFFSET(appId);
        fieldSize = 1u;
        break;   

    case Launcher_GET_BTLDB_ADDR:
        fieldPtr  = Launcher_MD_BTLDB_ADDR_OFFSET(appId);
    #if(!CY_PSOC3)
        fieldSize = 4u;
    #endif  /* (!CY_PSOC3) */
        break;

    case Launcher_GET_BTLDR_LAST_ROW:
        fieldPtr  = Launcher_MD_BTLDR_LAST_ROW_OFFSET(appId);
        break;

    case Launcher_GET_BTLDB_LENGTH:
        fieldPtr  = Launcher_MD_BTLDB_LENGTH_OFFSET(appId);
    #if(!CY_PSOC3)
        fieldSize = 4u;
    #endif  /* (!CY_PSOC3) */
        break;

    case Launcher_GET_BTLDB_ACTIVE:
        fieldPtr  = Launcher_MD_BTLDB_ACTIVE_OFFSET(appId);
        fieldSize = 1u;
        break;

    case Launcher_GET_BTLDB_STATUS:
        fieldPtr  = Launcher_MD_BTLDB_VERIFIED_OFFSET(appId);
        fieldSize = 1u;
        break;

    case Launcher_GET_BTLDB_APP_VERSION:
        fieldPtr  = Launcher_MD_BTLDB_APP_VERSION_OFFSET(appId);
        break;

    case Launcher_GET_BTLDR_APP_VERSION:
        fieldPtr  = Launcher_MD_BTLDR_APP_VERSION_OFFSET(appId);
        break;

    case Launcher_GET_BTLDB_APP_ID:
        fieldPtr  = Launcher_MD_BTLDB_APP_ID_OFFSET(appId);
        break;

    case Launcher_GET_BTLDB_APP_CUST_ID:
        fieldPtr  = Launcher_MD_BTLDB_APP_CUST_ID_OFFSET(appId);
        fieldSize = 4u;
        break;
        
    case Launcher_GET_BTLDB_COPY_FLAG:
        fieldPtr = Launcher_MD_BTLDB_COPY_FLAG_OFFSET(appId);
        fieldSize = 1u;
        break;        

    case Launcher_GET_BTLDB_USER_DATA:
        fieldPtr = Launcher_MD_BTLDB_USER_DATA_OFFSET(appId);
        fieldSize = 4u;
        break;        
